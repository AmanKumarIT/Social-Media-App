  ...
  return (
    <Container maxWidth="xs" sx={{ mt: 10 }}>
      <Paper elevation={0} sx={{ p: 5, borderRadius: 4, boxShadow: '0 10px 40px rgba(0,0,0,0.08)' }}>
        ...
        <Typography variant="h6" align="center" sx={{ mb: 3, fontWeight: 'bold', color: 'text.secondary' }}>Welcome Back</Typography>
        {error && <Alert severity="error" sx={{ mb: 2, borderRadius: 2 }}>{error}</Alert>}
        <Box component="form" onSubmit={handleSubmit} sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          <TextField required fullWidth label="Email Address" name="email" type="email" autoFocus value={formData.email} onChange={handleChange} InputProps={{ sx: { borderRadius: 3 } }} />
          ...
          <Button fullWidth variant="text" onClick={() => navigate('/signup')} sx={{ color: 'text.secondary', fontWeight: 'bold' }}>
